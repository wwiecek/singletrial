...
maketable=function(par,b,se,est,L,U){
  tab=data.frame(method=c("naive","Bayes"),MSE=NA,bias=NA,coverage=NA)
  tab$MSE[1]=mean((par - b)^2)
  tab$MSE[2]=mean((par - est)^2)
...
kable(tab,caption=caption,label="tab:mu") %>% 
  add_header_above(header=c(" "=1,"all"=4,"significant only"=2)) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
ggsave("bias.pdf",width = 8, height = 5)
ggsave("bias.png",width = 8, height = 5,dpi=300)
```
...
ggsave("diff_error2.pdf",width = 8, height = 5)
ggsave("diff_error2.png",width = 8, height = 5,dpi=300)
```
